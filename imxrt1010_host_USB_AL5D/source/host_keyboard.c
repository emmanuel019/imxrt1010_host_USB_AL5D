...
static uint8_t add_on_rxBuffer_g[BUFFER_SIZE];
...
void ADD_ON_AllocateVar(usb_host_keyboard_instance_t *keyboardInstance) {
	keyboardInstance->add_on_rxBuffer = add_on_rxBuffer_g;
	keyboardInstance->add_on_rxIndex  = 0;
}

void ADD_ON_UpdateVar(usb_host_keyboard_instance_t *keyboardInstance, uint8_t key) {
	uint16_t circle_id  		 = keyboardInstance->add_on_rxIndex % BUFFER_SIZE;
	uint8_t *tmpBuffer 		     = keyboardInstance->add_on_rxBuffer;
	uint8_t shift                = keyboardInstance->shiftPressed;
	*(tmpBuffer + circle_id)     = g_HostKeyboardTable[key][shift];
	keyboardInstance->add_on_rxIndex++;
}
...
static void USB_HostKeyboardProcessBuffer(usb_host_keyboard_instance_t *keyboardInstance)
{
    ...
    for (currentIndex = 2; currentIndex < 8; ++currentIndex)
    {
        ...
        if (longIndex < 6) /* note: long pressed key is continuous */
        {
            ...
            {
            	ADD_ON_UpdateVar(keyboardInstance,key);
...
void USB_HostHidKeyboardTask(void *param)
{
    ...
    ADD_ON_AllocateVar(keyboardInstance);
